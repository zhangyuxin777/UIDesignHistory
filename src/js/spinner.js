...
/**
 * Created by bruce zhang on 2016/2/1.
 */
var centerSpinner = new Spinner();
var bottomSpinner = new Spinner();
var centerSpinnerIsShowing = false;
var bottomSpinnerIsShowing = false;
function showCenterSpinner() {

    if (!centerSpinnerIsShowing) {
        centerSpinnerIsShowing = true;
        var loadTop = $(window).height() / 2;
        var loadLeft = $(window).width() / 2;
        var opts = {
            lines: 8, // 花瓣数目
            length: 4, // 花瓣长度
            width: 4, // 花瓣宽度
            ...
            top: loadTop + "px", // spinner 相对父容器Top定位 单位 px
            left: loadLeft + "px"// spinner 相对父容器Left定位 单位 px
        };
        centerSpinner = new Spinner(opts);
        var sHeight = $(window).height() > window.screen.height ? $(window).height() : window.screen.height;
        var htmlStr = "<div id='loadingCenterSpinner'"
            + "style= 'position: fixed; background-color:#000;opacity: 0.3;top:0;width: " + $('body').width() + "px;height: " + sHeight + "px;'>"
            + "</div>"
        $("body").append(htmlStr);
        var target = $("body").get(0);
        centerSpinner.spin(target);
    }
}
function hideCenterSpinner() {
    $("div").remove("#loadingCenterSpinner");
    centerSpinner.spin();
    centerSpinnerIsShowing = false;
}

function showCenterTransparentBgSpinner() {
    ...
    centerSpinner = new Spinner(opts);
    var htmlStr = "<div id='loadingCenterTransparentBgSpinner'"
        + "style= 'position: absolute;top:0;width: 100%;height: " + $("body").height() + "px;'>"
        + "</div>"
    $("body").append(htmlStr);
...
function hideCenterTransparentBgSpinner() {
    $("div").remove("#loadingCenterTransparentBgSpinner");
    centerSpinner.spin();
}

function showBottomSpinner(div) {
    var htmlStr = "<div id='loadingBottomSpinner'style='height: 40px'></div>";
    $(div).append(htmlStr);
    var loadTop = $("#loadingBottomSpinner").position().top + 20;
    var loadLeft = $(window).width() / 2;
    var opts = {
        lines: 8, // 花瓣数目
        length: 5, // 花瓣长度
    ...
    bottomSpinner = new Spinner(opts);
    var target = $("#loadingBottomSpinner").get(0);
    bottomSpinner.spin(target);
}
function hideBottomSpinner() {
    $("div").remove("#loadingBottomSpinner");
    bottomSpinner.spin();
}
